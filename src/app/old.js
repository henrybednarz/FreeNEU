...
import { viewVariants, formOverlayVariants, pwaOverlayVariants } from '@/styles/views.js';
...
export default function Home() {
    ...
    const handleNotificationSubmit = async (formData) => {
        ...
        const result = await subscribeNotification(formData);
        setIsSubmittingNotification(false);
        if (result.success) {
            showNotification('Subscribed to notifications successfully!', 'success');
        } else {
            showNotification(result.error, 'error');
        }
    }

    const handleNotificationRemove = async (formData) => {
        if (isSubmittingNotification) { return }
        const result = await unsubscribeNotification(formData);
        setIsSubmittingNotification(false);
        if (result.success) {
            ...
            <AnimatePresence>
                ...
                >
                    <EmailAddForm
                        ...
                        onSubmitNotification={handleEmailFormSubmit}
                        onSubmitRemoveNotification={handleNotificationRemove}
                        submissionState={{
